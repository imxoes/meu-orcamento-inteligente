...
import { generateFinancialInsights, isOpenAIConfigured } from '@/lib/openai-utils'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const transactions = await prisma.transaction.findMany({
      ...
      take: 500 // Mais transações para análise mais completa
    })
    ...
    const investments = await prisma.investment.findMany({
      where: { userId },
      orderBy: { createdAt: 'desc' }
    }).catch(() => []) // Se a tabela não existir, retorna array vazio

    // Gerar insights usando OpenAI
    const result = await generateFinancialInsights(transactions, goals, investments)

    if (!result) {
      return NextResponse.json(
        { error: 'Failed to generate insights', message: 'Erro ao gerar insights. Tente novamente.' },
        { status: 500 }
      )
    }
