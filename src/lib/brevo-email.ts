/**
 * Configuração de email com Brevo (Sendinblue) API
 */
...
export async function sendBrevoEmail(options: BrevoEmailOptions): Promise<boolean> {
  try {
    ...
    const senderEmail = process.env.BREVO_SENDER_EMAIL || 'noreply@useorbi.app'
    const senderName = process.env.EMAIL_FROM_NAME || 'Meu Orçamento Inteligente'

    if (!apiKey) {
      console.log('Email simulation - Brevo API not configured:', options)
...
export async function sendVerificationEmailBrevo(email: string, token: string): Promise<boolean> {
...
export async function sendWelcomeEmailBrevo(email: string, name: string): Promise<boolean> {
